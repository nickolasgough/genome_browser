...
function messageFromClient(who, msgType, content) {
    ...
    } else if (msgType == "update_ids") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].update_ids(who);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].update_ids(who);
        }
    } else if (msgType == "view_moved") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].update_views(who, content.content);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].update_views(who, content.content);
        }
    } else if (msgType == "remove_id") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].remove_id(who);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].remove_id(who);
        }
    } else if (msgType == "mouse_position") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].update_mice(who, content.content);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].update_mice(who, content.content);
        }
    } else if (msgType == "gene_highlighted") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].highlighted(who, content.content);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].highlighted(who, content.content);
        }
    } else if (msgType == "clear_others") {
        // call the function for of you p5 with the data you want, data is stored in content.content
        if (content.targetRoom) {
            if (roomsJoined[content.targetRoom]) roomsJoined[content.targetRoom].clear_others(who, content.content);
        } else {
            if (connectedPeopleCanveses[easyrtc.idToName(who)]) connectedPeopleCanveses[easyrtc.idToName(who)].clear_others(who, content.content);
        }
    }
